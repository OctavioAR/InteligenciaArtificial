...
class Puzzle:
    ...
    def obtener_vecinos(self, estado):
        vecinos = []
        fila_vacia, columna_vacia = self.encontrar_posicion(estado, 0)
        
        movimientos = [(-1, 0), (1, 0), (0, -1), (0, 1)]  # arriba, abajo, izquierda, derecha
        for mov_fila, mov_columna in movimientos:
            nueva_fila = fila_vacia + mov_fila
            nueva_columna = columna_vacia + mov_columna
            if 0 <= nueva_fila < 3 and 0 <= nueva_columna < 3:
                nuevo_estado = [fila[:] for fila in estado] 
                nuevo_estado[fila_vacia][columna_vacia], nuevo_estado[nueva_fila][nueva_columna] = nuevo_estado[nueva_fila][nueva_columna], nuevo_estado[fila_vacia][columna_vacia]
        ...
        while lista_prioridad:
            ...
            for vecino in self.obtener_vecinos(estado_actual):
                if tuple(map(tuple, vecino)) not in visitados: # Si el vecino no ha sido visitado se agregará a la lista de prioridad
                    ...
                    costo_h = self.distancia_manhattan(vecino)
...
def main():
    ...
    if solucion:
        ...
        print(f"Tiempo de ejecucion: {fin_tiempo - inicio_tiempo:.6f} segundos")
    else:
